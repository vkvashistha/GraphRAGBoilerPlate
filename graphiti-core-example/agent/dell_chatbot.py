...
# Initialize indices and constraints before using search
async def initialize_graphiti():
    await client.build_indices_and_constraints()
    print("Indices and constraints initialized successfully")

# Function to fetch and display Neo4j schema information
async def fetch_schema_info():
    """Fetch and display Neo4j schema information including indices and constraints"""
    print("\n=== Neo4j Schema Information ===")
    
    # Get indices
    indices = await client.driver.execute_query(
        "SHOW INDEXES",
        {},
        database_="neo4j",
    )
    ...
    print("\n--- Indices ---")
    for record in indices.records:
        index_info = record.data()
        print(f"Index: {index_info.get('name', 'N/A')}")
        print(f"  Type: {index_info.get('type', 'N/A')}")
        print(f"  Owner: {index_info.get('owningConstraint', 'N/A')}")
        print(f"  State: {index_info.get('state', 'N/A')}")
        print(f"  Properties: {index_info.get('properties', [])}")
        print(f"  Labels/Types: {index_info.get('labelsOrTypes', [])}")
        print()
    
    # Get constraints
    constraints = await client.driver.execute_query(
        "SHOW CONSTRAINTS",
        {},
        database_="neo4j",
    )
    ...
    for record in constraints.records:
        constraint_info = record.data()
        print(f"Constraint: {constraint_info.get('name', 'N/A')}")
        print(f"  Type: {constraint_info.get('type', 'N/A')}")
        print(f"  For: {constraint_info.get('for', 'N/A')}")
        print(f"  Properties: {constraint_info.get('properties', [])}")
        print()
print("\n\nclient")
print(client)
print("\n\nclient")
